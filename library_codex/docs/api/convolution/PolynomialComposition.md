...
| [`composition(inner, outer, degree=None, mod=DEFAULT_MOD)`](../../../convolution/PolynomialComposition.py#L275) | 合計を固定した非負または正の整数列を一様に生成する。 | `inner`: 内側の多項式/FPS `g`<br>`outer`: 外側の多項式/FPS `f`<br>`degree`: 必要な係数数・次数上限。省略時: `None`<br>`mod`: 法。Noneの場合は整数上の演算。省略時: `DEFAULT_MOD` | list[int] — 合計がtotalになる長さpartsの非負または正の整数列 |
