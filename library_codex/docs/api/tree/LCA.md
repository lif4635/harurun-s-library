<!-- Generated by tools/build_api_reference.py; do not edit directly. -->
# `library_codex.tree.LCA`

instanceを直接呼んで最近共通祖先を求めるLCA。

- 計算量の目安: 構築 O(N)、query O(1)
- source: [`tree/LCA.py`](../../../tree/LCA.py)
- 公開API: function 0、class 1、method/property 2（Python protocol 1を含む）

## できること

- lca = LCA(tree, root) と構築し、lca(u, v)で最近共通祖先を取得できる。
- dist(u, v)で2頂点間の辺数距離を取得できる。
- 森にも対応し、異なる連結成分の2頂点には-1を返す。

## Import

```python
from library_codex.tree.LCA import LCA
```

## Class `LCA`

Euler tourとRMQでLCAをO(1)で返す。

- constructor: [`LCA(tree, root=0)`](../../../tree/LCA.py#L11)
- 引数: `tree`: 各頂点の隣接頂点を並べた隣接list<br>`root`: 最初に根とする0-indexed頂点番号。省略時: `0`
- 返り値: `LCA` instance

| method / property | 種別 | 用途 | 引数 | 返り値 |
| --- | --- | --- | --- | --- |
| [`__call__(first, second)`](../../../tree/LCA.py#L19) | method | firstとsecondのLCAを返す。異なる連結成分なら-1。O(1)。 | `first`: 第1の0-indexed頂点番号<br>`second`: 第2の0-indexed頂点番号 | int — firstとsecondの最近共通祖先。異なる連結成分なら-1 |
| [`dist(first, second)`](../../../tree/LCA.py#L23) | method | firstとsecondの辺数距離を返す。異なる連結成分なら-1。O(1)。 | `first`: 第1の0-indexed頂点番号<br>`second`: 第2の0-indexed頂点番号 | int — firstとsecondの辺数距離。異なる連結成分なら-1 |
