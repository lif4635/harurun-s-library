...
| [`xor_min(value, version=-1)`](../../../ordered_set/PersistentBinaryTrie.py#L81) | method | XOR最小を求める。 | `value`: 追加・設定・問い合わせる値<br>`version`: 参照するversion番号。省略時: `-1` | `self.kth(0, version, value)` | O(B)（Bは管理するbit幅） |
