<!-- Generated by tools/build_api_reference.py; do not edit directly. -->
# `library_codex.graph.ConnectedComponents`

無向グラフの連結成分番号と成分ごとの頂点を求める。

- 計算量の目安: 各操作の計算量はAPI表を参照
- source: [`graph/ConnectedComponents.py`](../../../graph/ConnectedComponents.py)
- 公開API: function 1、class 0、method/property 0（Python protocol 0を含む）

## できること

- `connected_components`: `connected`・連結成分を求める。

## Import

```python
from library_codex.graph.ConnectedComponents import connected_components
```

## Functions

| signature | 用途 | 引数 | 返り値 |
| --- | --- | --- | --- |
| [`connected_components(graph)`](../../../graph/ConnectedComponents.py#L3) | `connected`・連結成分を求める。 | `graph`: 隣接listまたはグラフobject | tuple(連結成分番号（数値または入力要素型）, `groups`（list）) |
