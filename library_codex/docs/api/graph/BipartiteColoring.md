<!-- Generated by tools/build_api_reference.py; do not edit directly. -->
# `library_codex.graph.BipartiteColoring`

無向グラフが二部グラフか判定し、2色の割当を返す。

- 計算量の目安: 各操作の計算量はAPI表を参照
- source: [`graph/BipartiteColoring.py`](../../../graph/BipartiteColoring.py)
- 公開API: function 1、class 0、method/property 0（Python protocol 0を含む）

## できること

- `bipartite_coloring`: `bipartite`・彩色を求める。

## Import

```python
from library_codex.graph.BipartiteColoring import bipartite_coloring
```

## Functions

| signature | 用途 | 引数 | 返り値 |
| --- | --- | --- | --- |
| [`bipartite_coloring(graph)`](../../../graph/BipartiteColoring.py#L5) | `bipartite`・彩色を求める。 | `graph`: 隣接listまたはグラフobject | 各頂点の色を格納したlist[int]（数値または入力要素型） / `None` |
