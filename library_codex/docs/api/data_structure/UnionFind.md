...
- constructor: [`UnionFind(size)`](../../../data_structure/UnionFind.py#L6)
- 引数: `size`: 要素数・universe size
- 返り値: `UnionFind` instance

| method / property | 種別 | 用途 | 引数 | 返り値 |
| --- | --- | --- | --- | --- |
| [`find(node)`](../../../data_structure/UnionFind.py#L11) | method | 代表元・位置・対象要素を探す。 | `node`: 頂点・内部node番号 | 代表元・位置・node番号（int） |
| [`merge(first, second)`](../../../data_structure/UnionFind.py#L25) | method | 2要素・2成分・2構造を併合する。 | `first`: 第1入力・左側の値<br>`second`: 第2入力・右側の値 | `first` |
