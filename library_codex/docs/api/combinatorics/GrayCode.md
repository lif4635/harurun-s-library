...
# `library_codex.combinatorics.GrayCode`

整数とGray codeを相互変換する。

- source: [`combinatorics/GrayCode.py`](../../../combinatorics/GrayCode.py)
...
```python
from library_codex.combinatorics.GrayCode import gray_code, inverse_gray_code, gray_code_path
```
...
| --- | --- | --- | --- | --- |
| [`gray_code(value)`](../../../combinatorics/GrayCode.py#L5) | gray codeを求める。 | `value`: 追加・設定・問い合わせる値 | 数値または入力要素型 `value ^ value >> 1` | O(1) Python integer operations |
| [`inverse_gray_code(value)`](../../../combinatorics/GrayCode.py#L8) | inverse gray codeを計算する。 | `value`: 追加・設定・問い合わせる値 | 計算結果（int） | O(log value) shifts/xors |
