...
| [`fps_add(first, second, mod=DEFAULT_MOD)`](../../../fps/FormalPowerSeries.py#L58) | 2つの形式的冪級数を係数ごとに加算する。 | `first`: 第1入力・左側の値<br>`second`: 第2入力・右側の値<br>`mod`: 法。Noneの場合は整数上の演算。省略時: `DEFAULT_MOD` | list[number] — 2つの入力と同じ法上の和を表す昇べき順係数列。 |
