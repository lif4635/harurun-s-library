...
- 引数: `min_heap`: min heapの下限。省略時: `True`
- 返り値: `SkewHeap` instance
- 計算量: —

| method / property | 種別 | 用途 | 引数 | 返り値 | 計算量 |
| --- | --- | --- | --- | --- | --- |
| [`new_node(key, value=None)`](../../../sequence_structure/SkewHeap.py#L16) | method | new nodeを求める。 | `key`: 比較・格納に使うkey<br>`value`: 追加・設定・問い合わせる値。省略時: `None` | `node` | O(1) |
| [`meld(first, second)`](../../../sequence_structure/SkewHeap.py#L42) | method | meldを求める。 | `first`: 第1入力・左側の値<br>`second`: 第2入力・右側の値 | `second` / `first` / `root` | 償却 O(log N) |
