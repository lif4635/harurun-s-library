...
| [`query(left, bottom, right, top)`](../../../spatial_structure/DynamicPointAddRectangleSum.py#L14) | method | 指定した対象への問い合わせ結果を返す。 | `left`: 半開区間の左端（含む）<br>`bottom`: 矩形の下端（含まない）<br>`right`: 半開区間の右端（含まない）<br>`top`: 矩形の上端（含む） | `None` |
