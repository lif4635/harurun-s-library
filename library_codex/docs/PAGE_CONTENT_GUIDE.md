...
- 複数の返り値やtuple・listの構成要素を説明するときは、一つの長い段落にせず`名前: 意味`の箇条書きにする。

型を見れば分かる内容を繰り返すより、利用者が次に書くコードを決められる情報を優先します。

## 使用例と注意点

- 短い配列・辺list・隣接listは一行で示す。横幅に収まらない例だけ意味のまとまりごとに改行する。
- 配列を一要素ずつ縦へ並べ、説明より多くの高さを使わない。
...
特に、返り値を式で一意に定められるAPIでは「計算後の列」「指定範囲の集計結果」で済ませず、添字との対応まで書きます。たとえば区間和は`$\sum_{i=\mathrm{left}}^{\mathrm{right}-1}a_i$`、min-plus畳み込みは`$c_k=\min_{i+j=k}(a_i+b_j)$`のようにします。式の前後には、その式が返り値全体・`result[k]`・条件のどれを表すかを短い文章で添えます。

Webサイトはraw HTMLを説明から実行せず、MarkdownとTeXだけを描画します。GitHubの生成APIリファレンスにも同じ原文を載せます。

Webサイトの数式はMathMLと`STIX Two Math`で表示します。本文へ溶け込みすぎず、Computer Modernほど古典的にならない字形を基準とし、moduleごとに数式フォントを変えません。数式の見た目を変更するときは、英字だけでなく添字、分数、総和、括弧も同じ式で確認します。
