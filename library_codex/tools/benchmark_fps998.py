...
def load_backend(name):
    if name == "specialized":
        ...
        return multiply, fps_inv, fps_exp
    from library_codex.fps.FormalPowerSeries import (
        fps_exponential,
        fps_inverse,
        fps_multiply,
    )
    return fps_multiply, fps_inverse, fps_exponential
...
def main():
    ...
    multiply, inverse, exponential = load_backend(args.backend)
    size = args.size
    ...
    total = multiply_seconds + inverse_seconds + exponential_seconds
    checksum = (
        product[size - 1]
        + reciprocal[size - 1]
